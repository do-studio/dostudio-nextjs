...
async function getData() {
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/works?filters[category][$eq]=digital-marketing&populate=*`,
    { cache: "no-store"}
  );
    ...
    <main className='min-h-screen w-full bg-white'>
        ...
        {workdata.data && workdata.data.length > 0 ? (
          workdata.data?.map(data=>(
            <Link href={`/our-works/digital-marketing/${data.attributes.slug}`} key={data.id}>
