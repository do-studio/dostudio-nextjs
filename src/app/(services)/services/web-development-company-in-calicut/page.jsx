...
import { FaHtml5, FaNodeJs, FaReact, FaRegUserCircle, FaShopify, FaWordpress } from "react-icons/fa";
    ...
    return (
        ...
        {faqs1.map((faq, index) => (
            <div key={index} className="mb-2">
                <button
                    className={`w-full text-left p-4 bg-gray-100 ${activeIndex1 === index && 'bg-primarygreen'} transition-all duration-200 rounded-xl flex justify-between items-center`}
...
const FaqComponent2 = () => {
    ...
    return (
        ...
        {faqs2.map((faq, index) => (
            <div key={index} className="mb-2">
                <button
                    className={`w-full text-left p-4 bg-gray-100 ${activeIndex2 === index && 'bg-primarygreen'} transition-all duration-200  rounded-xl  flex justify-between items-center`}
...
async function fetchData() {
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/webdesigns?&populate=*`,
    { cache: "no-store"}
  );
    ...
    return (
        ...
        <section className='w-11/12 xl:w-10/12 mx-auto min-h-screen pt-20 py-20 flex flex-col gap-10 xl:gap-20'>
            ...
            <div className='flex flex-col md:flex-row gap-5 gap-y-3'>
                <h1 className='md:basis-1/2 xl:basis-3/5 text-3xl xl:text-6xl font-black uppercase'>Web Development Company In Calicut</h1>
                <div className='md:basis-1/2 xl:basis-2/5 flex flex-col gap-3'>
                    <p className=' text-justify tracking-tighter'>We are a web development business based in Calicut. As a web development
            ...
            <div>
              <p className='xl:text-center tracking-tighter xl:tracking-normal text-justify text-xl xl:text-2xl font-semibold'>"We build websites that are user-friendly, presenting clean and simple layouts, and it
                only takes a few seconds for users to get where they are going. This sleekness of
            ...
            <div className='flex flex-col gap-10 xl:gap-20'>
              ...
              <div className='grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-x-3 gap-y-10'>
                  <div className='group border shadow-lg hover:shadow-xl duration-200 rounded-xl p-5 flex flex-col justify-center gap-2 relative'>
                      ...
                      <h4 className='uppercase font-bold pt-2'>html</h4>
                      <p className='text-sm'>Get clean, responsive HTML code that ensures your website looks fantastic and operates smoothly on any device.</p>
                  </div> 
                  <div className='group border shadow-lg hover:shadow-xl duration-200 rounded-xl p-5 flex flex-col justify-center gap-2 relative'>
                      <div className='group-hover:scale-90 duration-300 h-12 w-12 rounded-full bg-black text-primarygreen  absolute -top-7 shadow-2xl shadow-black grid place-items-center'><FaWordpress className='text-3xl '/></div>
                      <h4 className='uppercase font-bold pt-2'>wordpress</h4>
                      <p className='text-sm'>Enjoy custom WordPress themes and seamless plugin integration for a dynamic and user-friendly website experience.</p>
                  </div> 
                  <div className='group border shadow-lg hover:shadow-xl duration-200 rounded-xl p-5 flex flex-col justify-center gap-2 relative'>
                      <div className='group-hover:scale-90 duration-300 h-12 w-12 rounded-full bg-black text-primarygreen  absolute -top-7 shadow-2xl shadow-black grid place-items-center'><FaShopify className='text-3xl '/></div>
                      <h4 className='uppercase font-bold pt-2'>Shopify</h4>
                      <p className='text-sm'>A Shopify website is an e-commerce platform that allows individuals and businesses to create and manage online stores.</p>
                  </div> 
                  <div className='group border shadow-lg hover:shadow-xl duration-200 rounded-xl p-5 flex flex-col justify-center gap-2 relative'>
                      <div className='group-hover:scale-90 duration-300 h-12 w-12 rounded-full bg-black text-primarygreen  absolute -top-7 shadow-2xl shadow-black grid place-items-center'><FaNodeJs className='text-3xl '/></div>
                      <h4 className='uppercase font-bold pt-2'>react js</h4>
                      <p className='text-sm'>Experience high-performance web apps with React.js, featuring interactive and responsive user interfaces.</p>
                  </div> 
                  <div className='group border shadow-lg hover:shadow-xl duration-200 rounded-xl p-5 flex flex-col justify-center gap-2 relative'>
                      <div className='group-hover:scale-90 duration-300 h-12 w-12 rounded-full bg-black text-primarygreen  absolute -top-7 shadow-2xl shadow-black grid place-items-center'><FaReact className='text-3xl '/></div>
                      <h4 className='uppercase font-bold pt-2'>node js</h4>
